...
Fluid::Fluid(uint width, uint height, float dx){
    ...
    cudaMalloc(&particles1_CUDA, width * height * sizeof(Particle));
    cudaMalloc(&particles2_CUDA, width * height * sizeof(Particle));
}

Fluid::~Fluid(){

    cudaFree(particles1_CUDA);
    cudaFree(particles2_CUDA);
